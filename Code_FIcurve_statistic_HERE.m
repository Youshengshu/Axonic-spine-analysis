...
Fidx{1}=dir('DATA_FIcurve_20210729_P16_target-GFP_S3C1.mat');
%Fidx{2}=dir('DATA_FIcurve_OE*.mat');
%GroupName={'CTRL2'};%GroupName={'Ctrl1','OE'};

Num=3;%选择大部分细胞都会达到的一个AP Num
thAP=1;
...
%Binwidth=100;
Binwidth=50;
BinL=600:Binwidth:4000-Binwidth;
BinR=BinL+Binwidth;
...
%         pause
        ...
        Wantidx=find(ismember(spkFreq,Num*2)&(StimAmp(:)<800));
       %% 这个函数主要是看矩阵a中的数是不是矩阵b中的成员，是的话结果返回1，不是返回0；
