 ...
 spkwaveform_temp=[];
   ...
   waveformV_AP=waveformV(:,Marker_AP);
        [spkwaveform,ttspanspk,HW,AHP,Threshold,Amp,max_slope,min_slope,...
            HWidx,AHPidx, Thresholdidx,max_slopeidx,min_slopeidx,latencyidx]  = AP_Statistic(waveformV_AP(:,jj),peaktimeRecord(jj),tspan,1);
                ...
                latency=(ttspanspk(latencyidx)+peaktimeRecord(jj)-pretime)*1000;
                Latencytemp(:,jj)=latency;
...
%                 end
            ...
            spkparamRecord{7}(:,k)=mean(Latencytemp,2);
%            spkparamRecord{8}(:,k)=Latencytemp;
                             
    end

spkParamName={'HW', 'AHP', 'Threshold', 'Amp','max_slope','min_slope','latency'};
%
save('GroupName',...
    'spkparamRecord',...
     'spkParamName','FilenameRecord')
for kk=1:length(fidx)
    xlswrite('Results_chr2_NI_50V_S.xls',FilenameRecord,['spkParam_',GroupName{kk}],'A2')
    xlswrite('Results_chr2_NI_50V_S.xls',spkParamName,['spkParam_',GroupName{kk}],'B1')
    xlswrite('Results_chr2_NI_50V_S.xls',[spkparamRecord{1}(1,:)',...
                                             spkparamRecord{2}(1,:)',...
                                             spkparamRecord{3}(1,:)',...
                                             spkparamRecord{4}(1,:)',...
                                             spkparamRecord{5}(1,:)',...
                                             spkparamRecord{6}(1,:)',...
                                             spkparamRecord{7}(1,:)'],['spkParam_',GroupName{kk}],'B2')
